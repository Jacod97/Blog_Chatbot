...
class Pipeline:
    ...
    def is_active(self, session_id):
        try:
            self.cur.execute(
                "SELECT active FROM chatbot_state WHERE session_id=%s",
                (session_id,)
            )
    ...
    def record_question(self, session_id):
        if session_id in self.session_state:
            ...
            if self.session_state[session_id]["question_count"] >= 5:
                try:
                    self.cur.execute(
                        "UPDATE chatbot_state SET active=FALSE WHERE session_id=%s",
                        (session_id,)
                    )
                    self.conn.commit()
                except Exception as e:
                    self.conn.rollback()
                    print("[DB ERROR - record_question]", e)
